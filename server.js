...
// Diagnostic: check proxy IP AND verify sticky session keeps the same IP
app.get("/api/proxy-test", async (req, res) => {
  try {
    const sessionId = "test" + Math.random().toString(36).slice(2, 8);
    const r1 = await proxyRequest("https://api.ipify.org?format=json", sessionId);
    const r2 = await proxyRequest("https://api.ipify.org?format=json", sessionId);
    cookieJars.delete(sessionId);
    const ip1 = r1.body.toString("utf8");
    const ip2 = r2.body.toString("utf8");
    res.json({
      ok: true,
      call1: { ip: ip1, status: r1.status, elapsed: r1.elapsed },
      call2: { ip: ip2, status: r2.status, elapsed: r2.elapsed },
      stickySessionWorking: ip1 === ip2,
      note: ip1 === ip2
        ? "✅ Sticky session OK — same IP across requests, DataDome cookies will stay valid"
        : "❌ Sticky session FAILED — IPs differ, IPRoyal basic plan may not support _session-XXX modifier",
    });
  } catch (err) {
