...
// Pay-As-You-Go basic plan: bare USER:PASS only — no country/session modifiers
// (those require premium tier and cause 407 Proxy Authentication Required)
...
// Single-request CMA fetch via IPRoyal residential proxy.
// The 3-step session-aware dance was needed when we had no proxy and were
// trying to bluff DataDome from datacenter IPs. With a real residential IP,
// the export endpoint accepts a direct GET — no warm-up cookies required.
async function fetchCmaExport(exportUrl) {
  const sessionId = "s" + Math.random().toString(36).slice(2, 12);
  console.log("[CMA] Direct GET via residential proxy");

  const r = await proxyRequest(exportUrl, sessionId, {
    headers: {
  ...
  console.log("[CMA] Result:", r.status, "type:", r.contentType, "size:", r.body.length, "elapsed:", r.elapsed + "ms");

  // Cleanup session jar (free memory)
  cookieJars.delete(sessionId);

  if (r.status !== 200) {
    return { ok: false, status: r.status, reason: "Export request failed", body: r.body.toString("utf8", 0, 500) };
  }

  return { ok: true, status: r.status, body: r.body, contentType: r.contentType };
}
...
app.get("/api/cma", async (req, res) => {
  ...
  try {
    ...
    const responseData = {
      ...
      method: "iproyal-residential-direct", elapsed: totalElapsed,
    };
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    cmaCacheTTL: "30min",
    cmaMethod: "iproyal-residential-direct",
  });
});

// Diagnostic: check what IP the proxy uses
app.get("/api/proxy-test", async (req, res) => {
  try {
    const sessionId = "test" + Math.random().toString(36).slice(2, 8);
    const r = await proxyRequest("https://api.ipify.org?format=json", sessionId);
    cookieJars.delete(sessionId);
    res.json({
      ok: true,
      proxyResponse: r.body.toString("utf8"),
      status: r.status,
      elapsed: r.elapsed,
      note: "This is the IP that CMA CGM will see",
    });
  } catch (err) {
