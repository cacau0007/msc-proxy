...
function parseCmaSchedulesV2(rawHtml, polName, podName, polCode, podCode) {
  ...
  for (const block of blocks) {
    ...
    // Extract T/S port names from "via XXX" text inside transshipment divs.
    // Can appear multiple times if the route has multiple transships.
    const tsMatches = [...block.matchAll(/class\s*=\s*"[^"]*transshipment[^"]*"[^>]*>[\s\S]*?via\s*(?:<[^>]*>)?\s*([^<,]+?)(?:,[^<]*)?(?:<|$)/gi)];
    const tsPortNames = tsMatches.map(m => m[1].trim()).filter(Boolean);
    ...
    // Build portCalls for chronological Excel expand mode. CMA HTML doesn't
    // give us per-port intermediate arrival/departure dates (those come from
    // a separate details endpoint), but at minimum we can list the TS ports
    // between POL and POD. Better than nothing for Excel expand mode.
    const portCalls = [];
    portCalls.push({ code: polCode, name: polName, arr: "", dep: etdRaw || "" });
    tsPortNames.forEach(n => portCalls.push({ code: "", name: n, arr: "", dep: "" }));
    portCalls.push({ code: podCode, name: podName, arr: etaRaw || "", dep: "" });
