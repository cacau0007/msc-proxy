...
// =====================================================================
// === CMA CGM via ZenRows API ===
// =====================================================================
// ZenRows is a scraping API specifically designed for sites with anti-bot
// protection (DataDome, Cloudflare, PerimeterX, Akamai). Unlike a proxy, it's
// a REST API: we GET https://api.zenrows.com/v1/?apikey=...&url=... with the
// right flags, they fetch the target and return the cleaned response.
//
// For DataDome on cma-cgm.com we need ALL of these flags together:
//   js_render=true       — execute the DataDome JS challenge inside their browser
//   premium_proxy=true   — use residential IPs (datacenter IPs are auto-blocked)
//   antibot=true         — enable advanced anti-bot bypass mode
//
// This combination costs ~25 credits per successful request.
// Free plan = 1000 credits = ~40 successful CMA fetches. Combined with our
// 30-min cache, repeat searches are free, so this stretches a long way.
//
// Set this env var in Railway:
//   ZENROWS_API_KEY = your ZenRows API key from https://app.zenrows.com/

const ZENROWS_API_KEY = process.env.ZENROWS_API_KEY || "";
const ZENROWS_ENDPOINT = "https://api.zenrows.com/v1/";

// Single-shot fetch through ZenRows. Returns { ok, status, body, contentType, elapsed }.
function zenrowsFetch(targetUrl) {
  return new Promise((resolve, reject) => {
    if (!ZENROWS_API_KEY) return reject(new Error("ZENROWS_API_KEY not configured"));

    // Build the ZenRows request URL with all the right flags for DataDome bypass
    const zenUrl = new URL(ZENROWS_ENDPOINT);
    zenUrl.searchParams.set("apikey", ZENROWS_API_KEY);
    zenUrl.searchParams.set("url", targetUrl);
    zenUrl.searchParams.set("js_render", "true");
    zenUrl.searchParams.set("premium_proxy", "true");
    zenUrl.searchParams.set("antibot", "true");
    // Skip loading useless resources in ZenRows' headless browser. DataDome
    // challenge pages have images/fonts/CSS we don't care about — blocking
    // them shaves 20-40% off the total request time without affecting bypass.
    zenUrl.searchParams.set("block_resources", "image,font,stylesheet,media,other");

    const startTime = Date.now();
    const u = new URL(zenUrl.toString());
    ...
    const req = https.request({
      ...
      timeout: 180000, // 180s — ZenRows takes 30-90s on heavy DataDome targets
    }, (res) => {
    ...
    req.on("error", reject);
    req.on("timeout", () => { req.destroy(); reject(new Error("ZenRows timeout (180s)")); });
    req.end();
  });
}

// === Cache to save ZenRows credits (each fetch costs ~25 credits) ===
...
app.get("/api/cma", async (req, res) => {
  ...
  if (!ZENROWS_API_KEY) {
    return res.status(500).json({ error: "ZenRows API not configured. Set ZENROWS_API_KEY env var in Railway." });
  }
  ...
  try {
    console.log("[CMA] Search via ZenRows:", pol, "->", pod, "weeks:", w);
    const startTotal = Date.now();

    const result = await zenrowsFetch(exportUrl);
    ...
    if (!result.ok) {
      const preview = result.body.toString("utf8", 0, 500);
      return res.status(502).json({
        error: "ZenRows fetch failed",
    ...
    if (!isXlsx) {
      ...
      return res.status(502).json({
        ...
        preview: result.body.toString("utf8", 0, 500),
      });
    }
    ...
    const responseData = {
      ...
      method: "zenrows-api", elapsed: totalElapsed,
    };
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    zenrowsConfigured: !!ZENROWS_API_KEY,
    zenrowsApiKey: ZENROWS_API_KEY ? ZENROWS_API_KEY.substring(0, 8) + "***" : "(not set)",
    cmaCacheSize: CMA_CACHE.size,
    cmaCacheTTL: "30min",
    cmaMethod: "zenrows-api",
  });
});

// Diagnostic: verify ZenRows API is reachable and the key is valid.
// Hits a simple test URL through ZenRows. If 200 + content, key works.
app.get("/api/proxy-test", async (req, res) => {
  try {
    const result = await zenrowsFetch("https://httpbin.org/ip");
    let ip = "";
    try {
    ...
    res.json({
      ...
      note: result.ok && ip
        ? "✅ ZenRows reachable, API key valid, returning a residential IP — ready to test CMA"
        : "❌ ZenRows API problem — check ZENROWS_API_KEY env var in Railway",
    });
  } catch (err) {
...
// Diagnostic: fetch ANY URL through ZenRows.
...
app.listen(PORT, () => {
  ...
  console.log("ZenRows API:", ZENROWS_API_KEY ? "CONFIGURED" : "NOT CONFIGURED");
});
