...
const PROXY_COUNTRY = process.env.PROXY_COUNTRY || "br"; // br = Brazil residential

// Build IPRoyal username with sticky session params
// Format: USERNAME_country-br_session-RANDOM_lifetime-10m
function buildProxyUser(sessionId) {
  let user = PROXY_USER;
  if (PROXY_COUNTRY) user += `_country-${PROXY_COUNTRY}`;
  if (sessionId) user += `_session-${sessionId}_lifetime-10m`;
  return user;
}
...
// Multi-step CMA fetch: visit main page → routing finder → export
async function fetchCmaExport(exportUrl) {
  const sessionId = "s" + Math.random().toString(36).slice(2, 12);
  console.log("[CMA] Session:", sessionId, "country:", PROXY_COUNTRY);
  ...
  if (r3.status !== 200) {
    return { ok: false, status: r3.status, reason: "Step 3 failed", body: r3.body.toString("utf8", 0, 500) };
  }
...
app.get("/api/cma", async (req, res) => {
  ...
  try {
    ...
    const responseData = {
      ...
      method: "iproyal-residential-multi-step", elapsed: totalElapsed,
    };
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    cmaCacheTTL: "30min",
    cmaMethod: "multi-step session-aware",
  });
});
