...
function scraperApiDownload(targetUrl, outFile, options = {}) {
  return new Promise((resolve, reject) => {
    const params = new URLSearchParams({
      ...
      url: targetUrl,
      premium: options.premium !== false ? "true" : "false",
      keep_headers: "true",
      country_code: options.country || "us",
    });
    const apiUrl = `https://api.scraperapi.com/?${params.toString()}`;

    console.log("[CMA] ScraperAPI request, premium:", options.premium !== false);
...
app.get("/api/cma", async (req, res) => {
  ...
  try {
    ...
    const result = await scraperApiDownload(exportUrl, tmpFile, { premium: true });
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    cmaMethod: "scraperapi-xlsx-export",
  });
});
