  ...
  for (const r of routings) {
    ...
    const legDetails = legs.map(l => {
      ...
      return {
        ...
        // Raw port codes + names — used to build portCalls[] below for
        // chronological Excel expand mode
        polCode: (s.location && (s.location.cityName || s.location.unLocode)) ? (s.location.unLocode || "") : "",
        polName: (s.location && s.location.cityName) || "",
        podCode: (e.location && (e.location.cityName || e.location.unLocode)) ? (e.location.unLocode || "") : "",
        podName: (e.location && e.location.cityName) || "",
      };
    });
    ...
    legs.forEach(l => {
      ...
      const pCode = (s.location && s.location.unLocode) || "";
      const pName = (s.location && s.location.cityName) || "";
      const dCode = (eLeg.location && eLeg.location.unLocode) || "";
      const dName = (eLeg.location && eLeg.location.cityName) || "";
