...
function normalizeHmmSchedule(r, polName, podName, polCode, podCode) {
  ...
  const serviceCode = r.mthVslCarrCd || r.laneCd || r.serviceCd || "";
  ...
  // HMM returns ETD/ETA under different names depending on the query view.
  // We try a priority list of field names.
  const etdRaw = r.porDptrDt || r.polDptrDt || r.departureDate || r.etd || "";
  const etaRaw = r.podArvDt || r.pvyArvDt || r.arrivalDate || r.eta || "";

  const departureDate = parseHmmDate(etdRaw);
  const arrivalDate = parseHmmDate(etaRaw);

  // Transit time: either provided directly or computed from dates
  let transitTime = parseInt(r.trnsTime || r.transitTime || "0", 10);
  ...
  // being null to indicate direct service
