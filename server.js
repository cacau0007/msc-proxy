...
app.get("/api/schedules", async (req, res) => {
  ...
  try {
    ...
    // Use tomorrow's date (MSC only shows from today/tomorrow)
    ...
    // Clear session on error so next request gets a fresh one
    sessionCookies = "";
    sessionTime = 0;
...
app.get("/api/health", (req, res) => {
  res.json({ status: "ok", hasSession: !!sessionCookies, sessionAge: sessionCookies ? Math.round((Date.now() - sessionTime) / 1000) + "s" : null });
});
