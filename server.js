...
const SCHEDULE_REGEX = new RegExp(
  ...
  "([^\\n]+)\\s+" +
  "Port Cut-off\\s+" +
  "(\\d{2}-[A-Z][a-z]{2}-\\d{4},\\s+\\d{2}:\\d{2}\\s+[AP]M)\\s+" +
  "Main vessel\\s+" +
  "([^\\n]+)\\s+" +
  "Service\\s+" +
  "([^\\n]+)\\s+" +
...
function parseCmaSchedules(rawHtml, polName, podName, polCode, podCode) {
  ...
  while ((match = SCHEDULE_REGEX.exec(text)) !== null) {
    const [, lateBooking, departureRaw, polRaw, polTerminalRaw, portCutoffRaw, vesselRaw, serviceRaw, voyageRefRaw, arrivalRaw, podRaw, podTerminalRaw, transitRaw, routeTypeRaw, co2Raw] = match;
