...
const MAERSK_BM_TELEMETRY = (process.env.MAERSK_BM_TELEMETRY || "")
  .replace(/[\r\n\t]+/g, "")
  .replace(/[\x00-\x1F\x7F]/g, "")
  .trim();
...
const HMM_COOKIES = (process.env.HMM_COOKIES || "")
  .replace(/[\r\n\t]+/g, "")
  .replace(/[\x00-\x1F\x7F]/g, "")
  .trim();
const HMM_CSRF_TOKEN = (process.env.HMM_CSRF_TOKEN || "")
  .replace(/[\r\n\t]+/g, "")
  .replace(/[\x00-\x1F\x7F]/g, "")
  .trim();
...
function hmmPost(url, bodyObj) {
  return new Promise((resolve, reject) => {
    ...
    req.on("error", reject);
    req.on("timeout", () => { req.destroy(); reject(new Error("HMM POST timeout (30s)")); });
    req.write(payload);
    req.end();
  });
}
...
// (same column set as Maersk/CMA normalization).
function normalizeHmmSchedule(r, polName, podName, polCode, podCode) {
  ...
  // Service code: HMM stores it in vslSvcLoopCd (e.g. "FIL", "FAL", etc)
  const serviceCode = r.vslSvcLoopCd || r.mthVslCarrCd || r.laneCd || r.serviceCd || "";
  const callSign = r.callSgnNo || r.callSign || "";

  // Origin/destination facility info from the record (falls back to params)
  ...
  // ETD/ETA: HMM nests the actual dates inside the `transit[]` array of legs.
  // Each leg has `dpartFnshDt` (departure finish) and `arvlStDt` (arrival start).
  // For end-to-end dates we take the FIRST leg's departure and LAST leg's arrival.
  let etdRaw = r.porDptrDt || r.polDptrDt || r.departureDate || r.etd || r.dpartFnshDt || "";
  let etaRaw = r.podArvDt || r.pvyArvDt || r.arrivalDate || r.eta || r.arvlStDt || "";

  const transitLegs = Array.isArray(r.transit) ? r.transit : [];
  if (!etdRaw && transitLegs.length > 0) {
    const first = transitLegs[0];
    etdRaw = first.dpartFnshDt || first.dpartStDt || first.polDptrDt || "";
  }
  if (!etaRaw && transitLegs.length > 0) {
    const last = transitLegs[transitLegs.length - 1];
    etaRaw = last.arvlStDt || last.arvlFnshDt || last.podArvDt || "";
  }

  const departureDate = parseHmmDate(etdRaw);
  const arrivalDate = parseHmmDate(etaRaw);

  // Transit time: HMM provides `trnstDwlTmHrs` in HOURS; convert to days.
  // Fall back to computed difference if direct fields aren't present.
  let transitTime = 0;
  if (transitLegs.length > 0) {
    // Sum all transit legs' dwell hours (main voyage + any transfers)
    const totalHours = transitLegs.reduce((acc, leg) => {
      const h = parseInt(leg.trnstDwlTmHrs || leg.trnstTmHrs || "0", 10);
      return acc + (isNaN(h) ? 0 : h);
    }, 0);
    if (totalHours > 0) transitTime = Math.round(totalHours / 24);
  }
  if (!transitTime) {
    transitTime = parseInt(r.trnsTime || r.transitTime || "0", 10);
  }
  if (!transitTime && departureDate && arrivalDate) {
    transitTime = Math.round((new Date(arrivalDate) - new Date(departureDate)) / 86400000);
  }

  // Direct vs transshipment — HMM uses fstTrshpPortCd/scndTrshpPortCd/thrdTrshpPortCd
  // being null to indicate direct service. Also transit[] with length > 1 means TS.
...
app.get("/api/hmm", async (req, res) => {
  ...
  try {
    ...
    if (step1.status !== 200 || isStep1Block) {
      return res.status(502).json({
        error: isStep1Block
          ? "HMM blocked the request (HTTP 403 / Akamai Bot Manager) — refresh HMM_COOKIES and HMM_CSRF_TOKEN env vars in Railway."
    ...
    if (step2.status !== 200 || isStep2Block) {
      return res.status(502).json({
        error: isStep2Block
          ? "HMM blocked step 2 (HTTP 403) — likely Akamai. Refresh env vars."
    ...
    res.status(502).json({ error: "HMM failed: " + err.message });
  }
});
