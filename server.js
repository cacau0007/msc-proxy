...
function parseCmaSchedules(rawHtml, polName, podName, polCode, podCode) {
  ...
  // Fallback parser: if the legacy regex found nothing, try the newer
  // "wrap-routes" HTML layout that CMA started emitting for some routes
  // (observed first on Shanghai → Salvador in Apr 2026). Does NOT replace
  // the legacy parser because routes like Shanghai → Navegantes still use
  // the old layout and work fine. Only runs when the primary parser whiffed.
  if (sailings.length === 0) {
    const fallback = parseCmaSchedulesV2(rawHtml, polName, podName, polCode, podCode);
    if (fallback.length > 0) {
      console.log("[CMA] primary parser returned 0, v2 (wrap-routes) found", fallback.length);
      sailings.push(...fallback);
    }
  }

  // Enrich V1 sailings with portCalls (TS ports + dates) extracted from the
  // raw HTML's <li class="transhipment"> blocks. V1's regex-on-text approach
  // loses structural context, so we walk the raw HTML by <div class="wrap-routes">
  // schedule blocks, pull TS ports with their arr/dep dates, and match back to
  // the sailing by vessel+voyage. This mirrors what V2 does, applied post-hoc.
  if (sailings.length > 0) {
    const wrapBlocks = [...rawHtml.matchAll(/<div\s+class\s*=\s*"[^"]*wrap-routes[^"]*"[^>]*>([\s\S]*?)(?=<div\s+class\s*=\s*"[^"]*wrap-routes|<\/div>\s*<\/div>\s*<\/div>\s*$)/gi)];
    for (const wm of wrapBlocks) {
      const block = wm[1];
      // Identify which sailing this block belongs to via Main vessel + Voyage Ref
      const vsMatch = block.match(/Main vessel[\s\S]{0,200}?<dd[^>]*>([^<]+)<\/dd>/i);
      const vgMatch = block.match(/Voyage Ref\.[\s\S]{0,200}?<dd[^>]*>[\s\S]*?>([^<]+)<\/[a-z]/i)
                   || block.match(/Voyage Ref\.[\s\S]{0,200}?<dd[^>]*>([^<]+)<\/dd>/i);
      const blockVessel = (vsMatch && vsMatch[1] || "").trim().toUpperCase();
      const blockVoyage = (vgMatch && vgMatch[1] || "").trim().toUpperCase();
      if (!blockVessel && !blockVoyage) continue;
      const target = sailings.find(s => {
        const vesselMatch = blockVessel && s.vessel && s.vessel.toUpperCase().includes(blockVessel.slice(0, 10));
        const voyageMatch = blockVoyage && s.voyageRef && s.voyageRef.toUpperCase() === blockVoyage;
        return vesselMatch || voyageMatch;
      });
      if (!target) continue;
      // Extract TS ports with arr/dep dates from <li class="transhipment">
      const tsLiMatches = [...block.matchAll(/<li\s+class\s*=\s*"[^"]*transhipment[^"]*"[^>]*>([\s\S]*?)<\/li>/gi)];
      const tsPorts = [];
      for (const m of tsLiMatches) {
        const liContent = m[1];
        const dateMatches = [...liContent.matchAll(/<span[^>]*class\s*=\s*"[^"]*date[^"]*"[^>]*>([^<]+)<\/span>/gi)];
        let portName = "", portCode = "";
        const nameMatch = liContent.match(/<a[^>]*capsule-container[^>]*>[\s\S]*?<\/span>\s*([^<\n]+?)(?:\s*<|,|$)/i);
        if (nameMatch) portName = nameMatch[1].trim();
        const codeMatch = liContent.match(/POLDescription=[^;]+%20;%20[A-Z]{2}%20;%20([A-Z0-9]+)/i);
        if (codeMatch) portCode = codeMatch[1];
        if (portName) {
          portName = portName.replace(/\s+/g, " ").replace(/,\s*([a-z]{2})\s*$/i, ", $1").toUpperCase().trim();
        }
        const etaRaw = dateMatches[0] ? dateMatches[0][1].trim() : "";
        const etdRaw = dateMatches[1] ? dateMatches[1][1].trim() : "";
        if (portName || portCode) tsPorts.push({ name: portName, code: portCode, arr: etaRaw, dep: etdRaw });
      }
      if (tsPorts.length === 0) continue;
      // Build portCalls: POL → [TS...] → POD
      target.portCalls = [
        { code: polCode, name: polName.toUpperCase(), arr: "", dep: target.departureDateDisplay || "" },
        ...tsPorts,
        { code: podCode, name: podName.toUpperCase(), arr: target.arrivalDateDisplay || "", dep: "" },
      ];
    }
  }
...
// --- V2 parser for CMA's newer "wrap-routes" HTML layout ---
// Each schedule lives in <div class="wrap-routes"> ... </div>. We split on
// that tag and extract fields with narrow regexes per-block, which is more
// forgiving than one big SCHEDULE_REGEX. Known fields in this layout:
//   data-event-date="MM/DD/YYYY" inside DepartureDatesCls → ETD
//   data-event-date="MM/DD/YYYY" inside ArrivalDatesCls   → ETA
//   <dt>Main vessel</dt><dd>NAME</dd>                     → vessel
//   <dt>Service</dt><dd><a ...>Long Name (CODE)</a></dd>  → service + code
//   <dt>Voyage Ref.</dt><dd><a ...>REF</a></dd>           → voyage
//   <dt>Port Cut-off</dt><dd>DATE, TIME</dd>              → port cutoff
//   data-value="NN" inside Transitcls                     → transit days
//   data-value="X.XX" inside TotalCo2Cls                  → CO2
//   <div class="transit transshipment...">via PORT</div>  → T/S indicator
function parseCmaSchedulesV2(rawHtml, polName, podName, polCode, podCode) {
  const sailings = [];
  // Split into per-schedule blocks. Each block starts at <div class="wrap-routes">
  // and ends just before the next one (or end of document).
  // Using a lookahead so we don't consume the delimiter.
  const blockRegex = /<div\s+class\s*=\s*"wrap-routes"[^>]*>[\s\S]*?(?=<div\s+class\s*=\s*"wrap-routes"|<\/body>|$)/gi;
  const blocks = rawHtml.match(blockRegex) || [];

  for (const block of blocks) {
    // --- Dates ---
    // Departure: find data-event-date inside a <li> with DepartureDatesCls class
    const etdMatch = block.match(/DepartureDatesCls[^>]*data-event-date\s*=\s*"([^"]+)"/i)
                   || block.match(/data-event-date\s*=\s*"([^"]+)"[^>]*DepartureDatesCls/i);
    const etaMatch = block.match(/ArrivalDatesCls[^>]*data-event-date\s*=\s*"([^"]+)"/i)
                   || block.match(/data-event-date\s*=\s*"([^"]+)"[^>]*ArrivalDatesCls/i);
    // Dates are in MM/DD/YYYY format — convert to YYYY-MM-DDTHH:MM format.
    // IMPORTANT: we append "T12:00:00" (local noon) to avoid the same timezone
    // shift bug that bit us on HMM/Maersk: date-only strings are parsed as UTC
    // midnight by `new Date(s)`, which in BRT renders as the previous day.
    // Using local noon makes this day-safe across all timezones.
    const etdRaw = etdMatch ? etdMatch[1] : "";
    const etaRaw = etaMatch ? etaMatch[1] : "";
    const toIso = (s) => {
      const m = (s || "").match(/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/);
      if (!m) return "";
      return `${m[3]}-${m[1].padStart(2,"0")}-${m[2].padStart(2,"0")}T12:00:00`;
    };
    const departureDate = toIso(etdRaw);
    const arrivalDate = toIso(etaRaw);

    // Skip malformed blocks (no dates = not a real schedule)
    if (!departureDate && !arrivalDate) continue;

    // --- Vessel, service, voyage, cutoff (dt/dd pairs) ---
    // Regex captures the <dd> content right after each known <dt> label.
    // Uses [\s\S] so it crosses newlines.
    const getDdAfter = (label) => {
      const re = new RegExp(`<dt>\\s*${label}\\s*<\\/dt>\\s*<dd>([\\s\\S]*?)<\\/dd>`, "i");
      const m = block.match(re);
      if (!m) return "";
      // Strip any nested tags to get clean text
      return m[1].replace(/<[^>]+>/g, "").replace(/\s+/g, " ").trim();
    };
    const vessel = getDdAfter("Main vessel");
    const serviceFull = getDdAfter("Service");
    const voyageRef = getDdAfter("Voyage Ref\\.");
    const portCutoff = getDdAfter("Port Cut-off");
    // Service code: extracted from parens like "Asia South America East Coast 3 (SEAS3)"
    const svcCodeMatch = serviceFull.match(/\(([^)]+)\)\s*$/);
    const serviceCode = svcCodeMatch ? svcCodeMatch[1] : serviceFull;

    // --- Transit days + CO2 ---
    const transitMatch = block.match(/Transitcls[^>]*data-value\s*=\s*"([^"]+)"/i)
                      || block.match(/data-value\s*=\s*"([^"]+)"[^>]*Transitcls/i);
    const co2Match = block.match(/TotalCo2Cls[^>]*data-value\s*=\s*"([^"]+)"/i)
                  || block.match(/data-value\s*=\s*"([^"]+)"[^>]*TotalCo2Cls/i);
    const transitTime = transitMatch ? (parseInt(transitMatch[1], 10) || 0) : 0;
    const co2 = co2Match ? `${co2Match[1]} CO2 (t)/TEU` : "";

    // --- Direct vs T/S ---
    // Presence of a "transit transshipment" div means T/S; absence means direct.
    const isDirect = !/class\s*=\s*"[^"]*transshipment/i.test(block);

    // --- Transshipment ports WITH dates ---
    // In the new CMA layout (routing-finder page), each T/S stop is in its
    // own <li class="transhipment"> containing 2 <span class="date"> tags:
    //   <li class="transhipment">
    //     <span class="date">Monday, 08-JUN-2026</span>     ← ETA at this TS
    //     <a ...>manzanillo, pa, PA</a>
    //     <span class="date">Saturday, 30-MAY-2026</span>   ← ETD from this TS
    //   </li>
    // Note the ETA appears BEFORE the ETD in the HTML order (matches how the
    // page shows it visually: the later date shown first, earlier second).
    //
    // Legacy layout uses "class='transit transshipment'" divs with "via PORT";
    // we fall back to that if the new layout finds nothing.
    const tsPorts = [];

    // Try new layout first: <li class="transhipment">
    const tsLiMatches = [...block.matchAll(/<li\s+class\s*=\s*"[^"]*transhipment[^"]*"[^>]*>([\s\S]*?)<\/li>/gi)];
    for (const m of tsLiMatches) {
      const liContent = m[1];
      // Two date spans inside: first = ETA, second = ETD
      const dateMatches = [...liContent.matchAll(/<span[^>]*class\s*=\s*"[^"]*date[^"]*"[^>]*>([^<]+)<\/span>/gi)];
      // Port name: extract from the <a> with capsule-container, or any anchor with city text
      let portName = "";
      let portCode = "";
      const nameMatch = liContent.match(/<a[^>]*capsule-container[^>]*>[\s\S]*?<\/span>\s*([^<\n]+?)(?:\s*<|,|$)/i);
      if (nameMatch) portName = nameMatch[1].trim();
      // Also try to extract place code from the href (e.g. POLDescription=MANZANILLO%20;%20PA%20;%20PAMIT)
      const codeMatch = liContent.match(/POLDescription=[^;]+%20;%20[A-Z]{2}%20;%20([A-Z0-9]+)/i);
      if (codeMatch) portCode = codeMatch[1];
      // Clean port name (they come as lowercase "manzanillo, pa, PA" → "MANZANILLO, PA")
      if (portName) {
        portName = portName.replace(/\s+/g, " ").replace(/,\s*([a-z]{2})\s*$/i, ", $1").toUpperCase().trim();
      }
      const etaRaw = dateMatches[0] ? dateMatches[0][1].trim() : "";
      const etdRaw = dateMatches[1] ? dateMatches[1][1].trim() : "";
      tsPorts.push({ name: portName, code: portCode, arr: etaRaw, dep: etdRaw });
    }

    // Fallback: legacy "via XXX" extraction if new layout didn't find any
    if (tsPorts.length === 0) {
      const legacyTs = [...block.matchAll(/class\s*=\s*"[^"]*transshipment[^"]*"[^>]*>[\s\S]*?via\s*(?:<[^>]*>)?\s*([^<,]+?)(?:,[^<]*)?(?:<|$)/gi)];
      legacyTs.forEach(m => tsPorts.push({ name: m[1].trim().toUpperCase(), code: "", arr: "", dep: "" }));
    }

    // --- Terminal names (POL/POD terminal links) ---
    // These come right after the capsule-container link in each ico-dot <li>.
    // Best-effort extraction; not critical if missing.
    const polTerminalMatch = block.match(/DepartureDatesCls[\s\S]*?<a[^>]*href="\/ebusiness\/schedules\/terminal\/[^"]*"[^>]*>([^<]+)<\/a>/i);
    const podTerminalMatch = block.match(/ArrivalDatesCls[\s\S]*?<a[^>]*href="\/ebusiness\/schedules\/terminal\/[^"]*"[^>]*>([^<]+)<\/a>/i);
    const polTerminal = polTerminalMatch ? polTerminalMatch[1].trim() : "";
    const podTerminal = podTerminalMatch ? podTerminalMatch[1].trim() : "";

    // Build portCalls chronologically: POL → [TS with dates] → POD
    const portCalls = [];
    portCalls.push({ code: polCode, name: polName.toUpperCase(), arr: "", dep: etdRaw || "" });
    tsPorts.forEach(ts => {
      portCalls.push({ code: ts.code, name: ts.name, arr: ts.arr, dep: ts.dep });
    });
    portCalls.push({ code: podCode, name: podName.toUpperCase(), arr: etaRaw || "", dep: "" });
    ...
    sailings.push({
      ...
      departureDate,
      arrivalDate,
      departureDateDisplay: etdRaw,
      arrivalDateDisplay: etaRaw,
      transitTime,
      co2,
      isDirect,
      portCutoff,
...
app.get("/api/cma", async (req, res) => {
  ...
  try {
    ...
    if (sailings.length === 0) {
      ...
      // but regex found zero schedules. Capture rich debug info so we can
      // figure out WHY the regex isn't matching — could be:
      //   - route-specific HTML variation (e.g. "via" prefix on T/S routes)
      //   - CMA changed the class names we anchor on
      //   - the response is a "please select intermediate port" interstitial
      const body = result.body;

      // 1. Count occurrences of common CMA schedule markers in the body.
      // These reveal what's present vs missing. If "Main vessel" is there
      // but the regex returns zero, our regex is the problem. If "Main
      // vessel" is missing too, the HTML is structurally different.
      const markerCounts = {
        "Main vessel":     (body.match(/Main vessel/g)        || []).length,
        "Port Cut-off":    (body.match(/Port Cut-off/g)       || []).length,
        "Via":             (body.match(/\bVia\b/g)            || []).length,
        "No direct":       (body.match(/No direct/gi)         || []).length,
        "Please select":   (body.match(/Please select/gi)     || []).length,
        "Departure Date":  (body.match(/Departure Date/g)     || []).length,
        "Transit time":    (body.match(/Transit time/g)       || []).length,
        "View all":        (body.match(/View all/g)           || []).length,
        "schedule-result": (body.match(/schedule-result/g)    || []).length,
        "routing-result":  (body.match(/routing-result/g)     || []).length,
        "data-route":      (body.match(/data-route/g)         || []).length,
      };

      // 2. Grab ~5 lines that look schedule-related so we can see the DOM
      // structure without having to dump the whole 600KB body.
      const scheduleSnippets = [];
      const lines = body.split("\n");
      for (const line of lines) {
        const t = line.trim();
        if (t.length < 30) continue;
        // Heuristic: lines with schedule-ish keywords are interesting
        if (/vessel|voyage|schedule|routing|departure|transit|port\s*of|call/i.test(t)) {
          // Compress whitespace to save log space
          scheduleSnippets.push(t.replace(/\s+/g, " ").substring(0, 400));
          if (scheduleSnippets.length >= 6) break;
        }
      }

      // 3. Also log a chunk from the MIDDLE of the body — the schedule cards
      // are usually in the center, not the <head> or <footer>.
      const midStart = Math.max(0, Math.floor(body.length / 2) - 800);
      const midChunk = body.substring(midStart, midStart + 1600).replace(/\s+/g, " ");

      console.log("[CMA] ZERO SAILINGS — debug dump:");
      console.log("[CMA]   bodySize:", body.length);
      console.log("[CMA]   markerCounts:", JSON.stringify(markerCounts));
      console.log("[CMA]   scheduleSnippets:", scheduleSnippets.length, "found");
      scheduleSnippets.forEach((s, i) => console.log(`[CMA]     [${i}] ${s}`));
      console.log("[CMA]   midChunk:", midChunk.substring(0, 500), "...");
      ...
      return res.status(502).json({
        ...
        elapsed: totalElapsed,
        hasScheduleMarkers,
        // Include rich debug info in the JSON response too, so it's
        // accessible via /api/cma?...&debug=1 without hitting Railway logs
        debug: {
          markerCounts,
          scheduleSnippets,
          bodyPreviewStart: body.substring(0, 1500),
          bodyPreviewMid: midChunk.substring(0, 1500),
          bodyPreviewEnd: body.substring(Math.max(0, body.length - 1500)),
        },
      });
    }
...
function normalizeMaerskRoutings(json, polName, podName, polCode, podCode) {
  ...
  for (const r of routings) {
    ...
    const legDetails = legs.map(l => {
      ...
      // Extract facility codes (always present for Maersk: CNNPOMT, MAPTMTM, BRSSATC etc)
      const polFac = (s.location && s.location.facility && s.location.facility.facilityCode) || "";
      const podFac = (e.location && e.location.facility && e.location.facility.facilityCode) || "";
      // unLocode and cityName may be missing — fall back to facility code
      const polUn = (s.location && s.location.unLocode) || "";
      const polCity = (s.location && s.location.cityName) || "";
      const podUn = (e.location && e.location.unLocode) || "";
      const podCity = (e.location && e.location.cityName) || "";
      return {
        ...
        polFacility: polFac,
        podFacility: podFac,
        // Port code: prefer unLocode, else facility code (Maersk frequently returns only facility)
        polCode: polUn || polFac,
        polName: polCity || polFac,
        podCode: podUn || podFac,
        podName: podCity || podFac,
      };
    });

    // portCalls = list of all ports the cargo touches in order, with
    // arrival/departure times. Built from legs: each leg contributes its POL
    // (with departure time) and POD (with arrival time). Adjacent legs share
    // a transshipment port — we dedupe but keep both arr (from prev leg's
    // podCall) and dep (from next leg's polCall).
    const portCalls = [];
    const seen = new Map(); // code -> index in portCalls
    const pushOrMerge = (code, name, arr, dep) => {
      if (!code) return;
      const idx = seen.get(code);
      if (idx == null) {
        portCalls.push({ code, name, arr: arr || "", dep: dep || "" });
        seen.set(code, portCalls.length - 1);
      } else {
    ...
    legs.forEach(l => {
      const c = l.carriage || {};
      const s = c.vesselPortCallStart || {};
      const eLeg = c.vesselPortCallEnd || {};
      // Same fallback logic: unLocode → facilityCode
      const pFac = (s.location && s.location.facility && s.location.facility.facilityCode) || "";
      const eFac = (eLeg.location && eLeg.location.facility && eLeg.location.facility.facilityCode) || "";
      const pCode = (s.location && s.location.unLocode) || pFac;
      const pName = (s.location && s.location.cityName) || pFac;
      const dCode = (eLeg.location && eLeg.location.unLocode) || eFac;
      const dName = (eLeg.location && eLeg.location.cityName) || eFac;
      pushOrMerge(pCode, pName, "", s.estimatedTimeOfDeparture || "");
      pushOrMerge(dCode, dName, eLeg.estimatedTimeOfArrival || "", "");
    });
    ...
    sailings.push({
      ...
      legs: legDetails,
      portCalls,  // for chronological Excel expand mode
...
app.get("/api/health", (req, res) => {
  res.json({
    ...
    maerskTrackBmTelemetryConfigured: !!process.env.MAERSK_TRACK_BM_TELEMETRY,
    maerskTrackBmTelemetryLength: (process.env.MAERSK_TRACK_BM_TELEMETRY || "").length,
    maerskTrackBmTelemetryEffectiveLength: (() => {
      let t = String(process.env.MAERSK_TRACK_BM_TELEMETRY || process.env.MAERSK_BM_TELEMETRY || "").trim();
      if (t.length > 5500) {
        const second = t.indexOf("a=", 5);
        if (second > 0) t = t.substring(0, second);
      }
      return t.length;
    })(),
...
app.post("/api/filters", (req, res) => {
  ...
  const clean = {
    ...
    routes: filter.routes.map(r => ({
      ...
      // Empty string = "Todos" (no service filter). Otherwise the substring
      // matched against sailing.service / serviceCode client-side.
      service: String(r.service || "").substring(0, 40),
      // Per-route toggle: if true, the Excel will include all transshipment
      // port calls (dates + times) for this route instead of just POL/POD.
      showAllTs: !!r.showAllTs,
    })),
  };
...
// =====================================================================
// === TRACKING — Universal booking/MBL/container tracking endpoints ===
// =====================================================================
// Frontend hits one of these with the user's reference and we fan out to
// all 9 carrier trackers in parallel. Stubs return { ok:false, notConfigured:true }
// until each carrier's API is wired up in trackers.js.

// List the carriers that have tracking slots available (used by frontend
// to render the "carriers checked" status indicator).
app.get("/api/track/carriers", (_req, res) => {
  res.json({
    carriers: Object.keys(trackers.TRACKER_REGISTRY).map(code => ({
      code,
      name: trackers.CARRIER_DISPLAY_NAMES[code] || code,
    })),
  });
});

// Tracking endpoint: type = "booking" | "mbl" | "cntr"
// Returns array of per-carrier results; frontend filters/displays.
async function handleTrackRequest(refType, req, res) {
  const ref = String(req.query.ref || "").trim();
  if (!ref) return res.status(400).json({ error: "Missing ref query param" });
  if (ref.length > 50) return res.status(400).json({ error: "Reference too long" });
  try {
    const results = await trackers.trackAcrossAllCarriers(refType, ref);
    // Sort: successful matches first, configured-but-failed next, not-configured last
    results.sort((a, b) => {
      const score = r => (r.ok ? 0 : r.notConfigured ? 2 : 1);
    ...
    console.error(`[track/${refType}] error:`, e);
    res.status(500).json({ error: e.message || "Tracking failed" });
  }
}

app.get("/api/track/booking", (req, res) => handleTrackRequest("booking", req, res));
app.get("/api/track/mbl",     (req, res) => handleTrackRequest("mbl",     req, res));
app.get("/api/track/cntr",    (req, res) => handleTrackRequest("cntr",    req, res));
