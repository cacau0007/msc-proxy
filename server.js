...
app.get("/api/cma", async (req, res) => {
  ...
  try {
    ...
    // --- Block detection: two tiers of DataDome rejection ---
    //
    // TIER 1 (classic): status 403 OR tiny body with explicit block markers.
    //   This catches hard rejections — clear-cut.
    //
    // TIER 2 (soft/stealth): status 200 but the response is a "no-content"
    //   placeholder page (observed size ~229KB) WITHOUT the markers that the
    //   real schedule page emits. DataDome and similar WAFs sometimes return
    //   a templated shell page with no results to avoid looking like a block.
    //   Heuristic: real schedule pages are 400KB+ (contain schedule cards);
    //   blocked/shell pages are in the 200-240KB band AND lack the two strings
    //   that ALWAYS appear when real schedules render: "Main vessel" and
    //   "Port Cut-off" (both are labels in every schedule card).
...
function normalizeHmmSchedule(r, polName, podName, polCode, podCode) {
  ...
  // Keep full datetime strings too (for display with hour granularity)
  const departureDateTime = etdRaw || "";
  const arrivalDateTime = etaRaw || "";
