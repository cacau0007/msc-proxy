...
// Pay-As-You-Go basic plan: ONLY accepts bare USER:PASS — no _country, _session,
// _lifetime, or any modifier (all return 407 Proxy Authentication Required).
// Confirmed via /api/proxy-test diagnostic.
const PROXY_HOST = process.env.PROXY_HOST || "";
const PROXY_PORT = process.env.PROXY_PORT || "";
const PROXY_USER = process.env.PROXY_USER || "";
const PROXY_PASS = process.env.PROXY_PASS || "";

// Build a fresh keepAlive proxy agent. Used per-flow so all requests in one
// CMA fetch share a single TCP connection to IPRoyal, which encourages the
// proxy to route them through the same backend residential IP — critical for
// DataDome cookie consistency since basic plan doesn't support sticky session.
function makeFlowAgent() {
  ...
  return new HttpsProxyAgent(proxyUrl, {
    ...
    maxSockets: 1, // Force all flow requests to serialize on the same socket
    maxFreeSockets: 1,
  });
}

// Legacy: stateless single-shot agent (used by /api/proxy-test diagnostic).
...
function proxyRequest(targetUrl, sessionId, options = {}) {
  return new Promise((resolve, reject) => {
    const agent = options.agent || getProxyAgent(sessionId);
...
// Strategy: create ONE keepAlive proxy agent and reuse it across all 3 requests.
// This forces all traffic through a single TCP connection to IPRoyal, which
// (we hope) makes the proxy route every request through the same backend
// residential IP — keeping DataDome cookies consistent. This is the only way
// to get pseudo-stickiness on IPRoyal basic plan, which rejects all username
// modifiers (no _session, no _country, no _lifetime — confirmed via /api/proxy-test).
async function fetchCmaExport(exportUrl) {
  const sessionId = "s" + Math.random().toString(36).slice(2, 12);
  const flowAgent = makeFlowAgent();
  if (!flowAgent) return { ok: false, status: 0, reason: "Proxy not configured" };
  console.log("[CMA] Multi-step fetch via residential proxy (keepAlive flow agent), session:", sessionId);
  ...
  if (r1.status === 403 || r1.body.toString("utf8", 0, 200).includes("captcha")) {
    ...
    flowAgent.destroy && flowAgent.destroy();
...
app.get("/api/proxy-test", async (req, res) => {
  ...
  try {
    ...
    // Bonus: test keepAlive flow agent — hit ipify twice with the same agent
    // and check if IPRoyal pinned the same backend IP
    const flowAgent = makeFlowAgent();
    async function tryFlowAgent(label) {
      return new Promise((resolve) => {
        const startTime = Date.now();
        const r = https.request({
          hostname: "api.ipify.org", port: 443, path: "/?format=json",
          method: "GET", agent: flowAgent,
          headers: { "User-Agent": CHROME_UA }, timeout: 15000,
        }, (resp) => {
          const chunks = [];
          ...
          resp.on("end", () => {
            ...
            let ip = "";
            try { ip = JSON.parse(body).ip || ""; } catch (e) {}
            resolve({ label, status: resp.statusCode, ip, elapsed: Date.now() - startTime });
          });
        });
        r.on("error", (e) => resolve({ label, status: 0, ip: "", elapsed: Date.now() - startTime, error: e.message }));
        r.on("timeout", () => { r.destroy(); resolve({ label, status: 0, ip: "", elapsed: Date.now() - startTime, error: "timeout" }); });
        r.end();
      });
    }
    const k1 = await tryFlowAgent("keepAlive-A");
    const k2 = await tryFlowAgent("keepAlive-B");
    const k3 = await tryFlowAgent("keepAlive-C");
    flowAgent.destroy && flowAgent.destroy();
    const keepAliveSticky = k1.status === 200 && k2.status === 200 && k3.status === 200
      && k1.ip && k1.ip === k2.ip && k2.ip === k3.ip;
    ...
    else if (keepAliveSticky) recommendation = "✅ keepAlive trick works! Same IP across requests via shared TCP connection — multi-step CMA fetch should now succeed";
    else recommendation = "❌ IPRoyal basic plan rejects all session modifiers AND keepAlive trick failed — need plan upgrade or alternative approach";

    res.json({
      ok: true,
      results: [r1, r2a, r2b, r3a, r3b, r4, r5],
      keepAliveTest: { k1, k2, k3, allSameIp: keepAliveSticky },
      analysis: {
        ...
        keepAliveTrickWorks: keepAliveSticky,
      },
      recommendation,
    });
  } catch (err) {
