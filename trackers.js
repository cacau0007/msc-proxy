 ...
 * @param {object} data - parsed `Data` field from MSC response
 * @param {string} reference - the input reference
 * @param {string} refType - "booking" | "mbl" | "cntr"
 * @param {string} containerFilter - if set, restrict events to this container number
 */
function mscNormalizeResponse(data, reference, refType, containerFilter) {
  ...
  // Pick which container's events drive the timeline. For CNTR lookups, find
  // the matching container; for booking/MBL, take the first one.
  ...
  const events = (primary.Events || []).slice().sort((a, b) => (a.Order || 0) - (b.Order || 0));

  // === Find vessel info from events ===
  // Pick the LAST event with a vessel (typically the ETA event for the final
  // vessel/voyage). Fallback to any vessel event.
  ...
  const eta = arrivalEvent ? mscParseDate(arrivalEvent.Date) : mscParseDate(info.FinalPodEtaDate || primary.PodEtaDate || "");

  // === Determine status ===
  // The latest event (highest Order) tells current status; combined with
  // the BL's Delivered flag and "LatestMove" string.
  const latestEvent = events[events.length - 1];
  let status = "In transit";
  if (bl.Delivered || primary.Delivered) status = "Delivered";
  else if (primary.LatestMove) {
    // LatestMove is a port name; combine with latest event description
  ...
  // === Build legs from vessel transitions in events ===
  // Each (Detail[0], Detail[1]) pair = (vessel, voyage). When that pair
  // changes between events, a new leg starts. Walk events oldest-first.
  ...
  for (const ev of eventsAsc) {
    ...
    // Skip non-vessel events (Empty, Laden, etc) for leg construction
  ...
  // === Port calls (chronological, deduplicated by location code) ===
  const portCalls = [];
  const seen = new Map();
  ...
  for (const ev of eventsAsc) {
    ...
    // Intermediate port calls (e.g. SALVADOR before SANTOS)
    if (Array.isArray(ev.IntermediaryPortCalls)) {
      for (const ipc of ev.IntermediaryPortCalls) {
        pushPort(
          ...
          ipc.LocationName || "",
          "",
          mscParseDate(ipc.Eta),
          mscParseDate(ipc.Etd),
        );
      }
    }
  }

  // === Build events list (for the timeline UI) ===
  // We want chronological order (oldest → newest) with human-readable details.
  const evList = eventsAsc.map(ev => ({
    date: mscParseDate(ev.Date),
    description: ev.Description || "",
    location: ev.Location || "",
    vessel: ev.Detail && ev.Detail[0] !== "EMPTY" && ev.Detail[0] !== "LADEN" ? ev.Detail[0] : "",
  }));

  // === Container info (for cntr-type lookups) ===
  const container = {
    number: primary.ContainerNumber || "",
    type: primary.ContainerType || "",
  };
  ...
  if (etd && eta) {
    ...
    if (!isNaN(t1) && !isNaN(t2)) {
      transitDays = Math.round((t2 - t1) / 86400000);
    }
  }

  // === Parse origin/destination from PoL/PoD strings (e.g. "SHEKOU, CN") ===
  // Use legs to find canonical POL/POD codes — the first event might be the
  // empty-pickup at a different depot (e.g. CNXIS) instead of the actual POL.
  const splitLoc = s => {
  ...
  return {
    ...
    vessel: mainVessel || "—",
    voyage: mainVoyage || "",
    service: "",          // MSC tracking JSON doesn't return commercial service
    serviceCode: "",
    etd: etd || "",
    eta: eta || "",
    origin: polObj.name,
    ...
    portCalls,
    events: evList,
    container: refType === "cntr" || containers.length === 1 ? container : null,
    // For multi-container BL/booking, list all container numbers
    containerNumbers: bl.ContainersInfo && bl.ContainersInfo.length > 1
      ? bl.ContainersInfo.map(c => c.ContainerNumber).filter(Boolean)
      : [],
...
async function trackAcrossAllCarriers(refType, reference) {
  ...
  const results = await Promise.all(
    carriers.map(c => withTimeout(
      TRACKER_REGISTRY[c][refType](reference).then(r => ({ ...r, carrier: r.carrier || c })),
      12000,
      c
    ))
  );
