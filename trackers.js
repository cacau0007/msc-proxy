...
// CMA CGM
// ============================================================================
// CMA tracking POSTs to /ebusiness/tracking/search with form-urlencoded data
// and returns a full HTML page. Embedded inside the HTML is a `var model = [...]`
// JavaScript array containing all the tracking events for each container in
// the booking/BL/container being looked up.
//
// Same DataDome protection as CMA schedules → reuse the CMA_COOKIES env var
// already managed by server.js (via helpers.getCmaCookies/setCmaCookies).
//
// The shape of `model` (per container):
//   {
//     ContainerReference, Size, Type, SizeAndType,
//     MovementDate, Location, LocaltionTerminal,    // current/last move
//     EstimatedTimeOfArrival,
//     ContainerMoveDetails: {
//       containerSize, containerType, containerStatus,
//       routingInformation: { portOfLoading, portOfDischarge, ... },
//       references: { shipmentReference, blReference, containerReference, customNumber },
//       pastMoves: [...],
//       currentMoves: [...],
//       futureMoves: [...]
//     },
//     LaraContainerSize  // friendly equipment code (e.g. "40HC", "40RH")
//   }
//
// We map ONE entry → ONE result card, matching the multi-container pattern
// used by MSC. The same parser handles booking/MBL/cntr — only `SearchBy`
// changes in the POST body.

const CMA_TRACK_URL = "https://www.cma-cgm.com/ebusiness/tracking/search";

/** ISO-ish input "2026-04-30T04:30:00" → keep as-is; "0001-01-01T..." → "". */
function cmaCleanDate(s) {
  if (!s) return "";
  if (typeof s !== "string") return "";
  if (s.startsWith("0001-")) return ""; // .NET DateTime.MinValue marker
  return s;
}

/** Map CMA's containerStatus enum + status code into a human label. */
function cmaStatusLabel(status, code) {
  const s = String(status || "");
  const c = String(code || "").toUpperCase();
  // First check the explicit code which is more granular
  switch (c) {
  ...
  // Fall back to the verbose status name
...
/** Translate a CMA move's verbose description code into a readable string. */
function cmaMoveDescription(move) {
  const status = move.containerStatus || "";
  const code = move.containerStatusDescription || "";
  return cmaStatusLabel(status, code);
}

/** Calls the CMA tracking endpoint. Returns { html, error }. */
async function cmaFetchTracking(reference, searchBy) {
  const { httpPostForm, getCmaCookies, CMA_UA } = require("./helpers");
  const cookies = getCmaCookies();
  if (!cookies) {
    return { error: "CMA_COOKIES not configured" };
  }
  const result = await httpPostForm(CMA_TRACK_URL, {
    "SearchViewModel.SearchBy": searchBy,            // "Booking" | "BL" | "Container"
  ...
  if (result.status !== 200) {
    return { error: `CMA HTTP ${result.status}` };
  }
  const html = result.body || "";
  // DataDome interstitial page check — body is mostly JS challenge, no model.
  if (/<title>[^<]*captcha|datadome|access denied/i.test(html.substring(0, 2000))) {
    return { error: "CMA: DataDome blocked the request — refresh CMA_COOKIES" };
  }
  return { html };
}

/** Extract `var model = [...]` from the CMA tracking HTML. Returns array or null. */
function cmaExtractModel(html) {
  // The model array is multiline JSON-like, terminated by "];" before the
  // next <script> chunk. We capture the bracket contents only.
...
/** Build a single result card from one CMA container entry in the model. */
function cmaNormalizeContainer(entry, reference, refType) {
  ...
  // === Build legs from XOF/AVD → AVA/IDF/TDF pairs ===
  // CMA gives one event per status change. A "leg" begins with a load (XOF/TOF)
  // and ends with a discharge (IDF/TDF). We walk allMoves chronologically and
  // pair them up; each pair becomes one leg.
  const legs = [];
  let openLeg = null;
  for (const move of allMoves) {
    ...
    if (isLoad) {
      ...
      openLeg = {
        ...
        podName: "",
        podTerminal: "",
        etd: cmaCleanDate(move.containerStatusDate),
        eta: "",
        imo: move.vesselId || "",
      };
      ...
      openLeg.eta = cmaCleanDate(move.containerStatusDate);
      legs.push(openLeg);
      openLeg = null;
    }
  }
  ...
  for (const move of allMoves) {
    ...
    const date = cmaCleanDate(move.containerStatusDate);
    if (code === "XOF" || code === "TOF" || code === "AVD" || code === "PVD") {
      ...
      // MOS/XRX/IFC/MEA → not strictly port calls but inform the location row
      pushPort(loc.code, loc.name, move.locationTerminal, "", "");
    }
  }

  // === Events list (timeline UI) — newest first ===
  const evList = allMoves.slice().reverse().map(move => ({
    date: cmaCleanDate(move.containerStatusDate),
    dateRaw: move.containerStatusDate || "",
    description: cmaMoveDescription(move),
    location: (move.location && move.location.name) || "",
    locationCode: (move.location && move.location.code) || "",
    vessel: move.vesselName || "",
    voyage: move.voyageReference || "",
    cargoState: "", // not tracked by CMA
    terminal: move.locationTerminal || "",
  }));

  // === Header info ===
  // Latest move (current → last past) determines status / vessel display.
  const latest = currentMoves[currentMoves.length - 1] || pastMoves[pastMoves.length - 1] || {};
  const status = cmaStatusLabel(details.containerStatus, latest.containerStatusDescription);
  ...
  const etd = cmaCleanDate(pol.date);
  const eta = cmaCleanDate(entry.EstimatedTimeOfArrival || pod.date);

  // direct = single vessel covering the whole route
  ...
  // CMA's location.name comes already split ("SHANGHAI" not "SHANGHAI, CN").
  // The country code is appended in parens in routingInformation only.
  ...
  return {
    ...
    etd: etd || "",
    eta: eta || "",
    origin: stripCountry(pol.name),
    originCode: pol.code || "",
    destination: stripCountry(pod.name),
    destinationCode: pod.code || "",
    status,
    isDirect,
    transitDays,
    ...
    latestMove: cmaMoveDescription(latest),
    podEtaDate: cmaCleanDate(entry.EstimatedTimeOfArrival),
...
/**
 * Normalize a parsed CMA model array into one or more result cards.
 * If refType="cntr", filter to the matching container only.
 */
function cmaNormalizeResponse(model, reference, refType) {
  if (!Array.isArray(model) || model.length === 0) {
    return { ok: false, error: "CMA: no container data", carrier: "CMA" };
  }
  let entries = model;
  if (refType === "cntr") {
    const match = model.find(e =>
      String(e.ContainerReference || "").toUpperCase() === String(reference).toUpperCase()
    );
    if (match) entries = [match];
  }
  const results = entries.map(e => cmaNormalizeContainer(e, reference, refType));
  return results.length === 1 ? results[0] : results;
}

async function cmaBookingTrack(reference) {
  const { html, error } = await cmaFetchTracking(reference, "Booking");
  if (error) return { ok: false, error, carrier: "CMA" };
  const model = cmaExtractModel(html);
  if (!model) return { ok: false, error: "CMA: tracking model not found in HTML", carrier: "CMA" };
  return cmaNormalizeResponse(model, reference, "booking");
}

async function cmaMblTrack(reference) {
  const { html, error } = await cmaFetchTracking(reference, "BL");
  if (error) return { ok: false, error, carrier: "CMA" };
  const model = cmaExtractModel(html);
  if (!model) return { ok: false, error: "CMA: tracking model not found in HTML", carrier: "CMA" };
  return cmaNormalizeResponse(model, reference, "mbl");
}

async function cmaCntrTrack(reference) {
  const { html, error } = await cmaFetchTracking(reference, "Container");
  if (error) return { ok: false, error, carrier: "CMA" };
  const model = cmaExtractModel(html);
  if (!model) return { ok: false, error: "CMA: tracking model not found in HTML", carrier: "CMA" };
  return cmaNormalizeResponse(model, reference, "cntr");
}
