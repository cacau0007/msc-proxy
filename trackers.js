...
//
// Akamai Bot Manager protects the API. The same MAERSK_BM_TELEMETRY env var
// used for schedules works here (single token covers the whole api.maersk.com
// domain). Without it we'll usually still get through from a real browser IP,
// but datacenter IPs (Railway) need it.
//
...
function _maerskGet(reference) {
  return new Promise((resolve, reject) => {
    ...
    const tel = String(process.env.MAERSK_BM_TELEMETRY || "").trim();
...
async function _maerskTrack(reference, refType) {
  let result;
  try {
    result = await _maerskGet(reference);
  } catch (e) {
  ...
  if (result.status === 403) {
    return { ok: false, error: "MAERSK HTTP 403 — refresh MAERSK_BM_TELEMETRY", carrier: "MAERSK" };
  }
  if (result.status !== 200) {
    return { ok: false, error: `MAERSK HTTP ${result.status}`, carrier: "MAERSK" };
  }
