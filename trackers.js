...
function _maerskGet(reference, cookies) {
  return new Promise((resolve, reject) => {
    ...
    let tel = String(process.env.MAERSK_TRACK_BM_TELEMETRY || process.env.MAERSK_BM_TELEMETRY || "").trim();
    // Defensive: if the env var was accidentally pasted twice (a single
    // valid telemetry is ~3500-5000 chars), trim everything after the first
    // complete payload. The format is "a=...&&&e=...&&&sensor_data=..." and
    // the second copy starts with another "a=" — cut right before that.
    if (tel.length > 5500) {
      const secondStart = tel.indexOf("a=", 5);
      if (secondStart > 0) tel = tel.substring(0, secondStart);
    }
